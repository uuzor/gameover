module virtualz::ivirtualz {
    ...
    public struct IVIRTUALZ has drop {}
    ...
    public struct TokenTreasury has key {
        id: UID,
        supply: Supply<IVIRTUALZ>,
    }

    /// Global game state
    public struct GameState has key {
        id: UID,
        treasury: Balance<IVIRTUALZ>,
        current_season_id: u64,
        total_bets_placed: u64,
        total_volume: u64,
        early_user_rewards_pool: Balance<IVIRTUALZ>,
    ...
    public struct Season has key, store {
        ...
        season_winner_pool: Balance<IVIRTUALZ>,
    ...
    fun init(witness: IVIRTUALZ, ctx: &mut TxContext) {
        // Create LEAGUE token using module witness
        let (treasury_cap, metadata) = coin::create_currency<IVIRTUALZ>(
            witness,
            9, // decimals
            b"LEAGUE",
            b"LEAGUE Token",
    ...
    public entry fun place_bet(
        ...
        let _treasury_amount = stake - early_user_amount;
    ...
    public fun get_leaderboard(_season: &Season): vector<u8> {
